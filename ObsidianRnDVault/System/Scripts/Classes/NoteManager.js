...
class NoteManager {
	...
	async createFromTemplate(
		...
		const tFolder = this.createFolderIfMissing(destinationFolder);
		const validNoteName = this.generateValidNoteName(tFolder, newNoteName);
		...
		const tFile = app.vault.getAbstractFileByPath(
			this.generateFullPathWithExt(tFolder, validNoteName),
		);
		return {
			template: template,
			tFile: tFile,
			noteName: validNoteName,
			openNote: openNewNote,
			tFolder: tFolder,
		};
	}
