...
class Init {
	...
	/**
	 * Attach the functions to the target, usally this.
	 *
	 * @param {*} target
	 * @memberof Init
	 */
	async attach(target) {
		...
		target.tp = await app.plugins.plugins["templater-obsidian"].templater
		...
		target.target.sleep = async (ms) => {
			await new Promise((resolve) => setTimeout(resolve, ms));
		};
		const noteCreator = this.importNoteCreator();
		await noteCreator.attatchFunctions(target);
	}
